"""
ball_finder_v10b.py
修复：exit arm 基于真实检测 box 接近边界，而不是 pred 出界判断
"""
import cv2
import numpy as np
...
OUTPUT_PATH = "/Users/harryzhang/git/tempopeak/outputs/ball_finder_v10b.mp4"
...
EXIT_RELINK_PX       = 120
EXIT_SEARCH_R        = 200
EDGE_ARM_MARGIN      = 80   # 真实 box 距边界多少 px 就 arm exit
...
def clamp_to_edge(box, W, H):
    cx,cy=center(box)
    cx=float(np.clip(cx,0,W)); cy=float(np.clip(cy,0,H))
    return cx,cy
        ...
        if dets:
            ...
            if exit_center is not None:
                ...
                if dist<=EXIT_RELINK_PX:
                    trk_box=best[:4]; trk_vel=np.zeros(2)
                    trk_vel_age=2; trk_coast=0; trk_coasting=False
                    exit_center=None; chosen=best
                    reset_cand()
                    print(f"  f{fi:04d} EXIT RELINK ✓ dist={dist:.0f}")
