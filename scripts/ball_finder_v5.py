...
OUTPUT_PATH = "/Users/harryzhang/git/tempopeak/outputs/ball_finder_v5.mp4"
...
def pick_best_det(dets, prev_box=None, prev_size=None):
    ...
    def score(d):
        ...
        # lower dist + lower sz_diff + higher squareness = better
...
# Track state
trk_box   = None   # current box
trk_vel   = np.zeros(2)
trk_prev_vel = np.zeros(2)
trk_coast = 0

frame_results = []   # per frame: (box, is_predicted) or None
frame_dv      = []
...
for fi, dets in enumerate(filtered):
    ...
    if dets:
        # pick best detection given current track state
        ...
        if trk_box is not None:
            # only accept if within link threshold
    ...
    if trk_box is not None:
        spd = float(np.linalg.norm(trk_vel))
        print(f"  f{fi:04d} {'P' if is_pred else 'R'} vel={spd:.1f} dv={dv:.1f}")
...
while True:
    ...
    if box is not None:
        ...
        label = "pred" if is_pred else "ball"
        cv2.putText(frame,label,(x1,y1-6),cv2.FONT_HERSHEY_SIMPLEX,0.5,color,lw)
