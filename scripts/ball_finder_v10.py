...
ball_finder_v10.py
fixes:
1. hit_signal = angle only (speed 只做门槛，不做乘数)
2. 出界后记录 exit 位置，120px 内重新出现直接接回
3. NEW_TRACK_MIN_FRAMES 只对全新 track 生效
"""
import cv2
import numpy as np
...
OUTPUT_PATH = "/Users/harryzhang/git/tempopeak/outputs/ball_finder_v10.mp4"
...
MIN_ANGLE_FOR_HIT    = 30.0      # 度，低于此角度不算 hit
COAST_DECAY          = 0.85
NEW_TRACK_MIN_FRAMES = 3
OUT_MARGIN           = 30
EXIT_RELINK_PX       = 120       # 出界后重新接回的搜索半径
...
def hit_signal(v1, v2, min_vel=MIN_VEL_FOR_HIT, min_angle=MIN_ANGLE_FOR_HIT):
    """纯角度信号：速度和角度都达标才返回角度值，否则 0"""
    n1=np.linalg.norm(v1); n2=np.linalg.norm(v2)
    if n1<0.5 or n2<min_vel: return 0.0
    cos_a=np.clip(np.dot(v1,v2)/(n1*n2),-1.0,1.0)
    angle=float(np.degrees(np.arccos(cos_a)))
    return angle if angle>=min_angle else 0.0
...
# exit relink
exit_box          = None   # 出界前最后一个 box

# candidate (新 track 缓冲)
cand_box          = None
cand_vel          = np.zeros(2)
cand_frames       = 0
        ...
        if chosen is None:
            if not trk_coasting and trk_coast<MAX_COAST and spd>0.5:
                ...
                if is_out_of_frame(trk_box,W,H):
                    print(f"  f{fi:04d} OUT → exit relink armed at {[int(c) for c in center(trk_box)]}")
                    exit_box=trk_box
                    reset_track(); is_pred=False
            else:
                reset_track()

    # ── no confirmed track: try exit relink first, then candidate ─────────
    if trk_box is None and chosen is None:
        if dets:
            best=max(dets,key=lambda d: d[4])
            bc=center(best[:4])

            # exit relink: 直接接回，不需要 MIN_FRAMES
            if exit_box is not None:
                dist=float(np.linalg.norm(bc-center(exit_box)))
                if dist<=EXIT_RELINK_PX:
                    trk_box    =best[:4]
                    trk_vel    =bc-center(exit_box)   # 粗略速度
                    trk_vel_age=2                      # 需要再积累几帧
                    trk_coast=0; trk_coasting=False
                    exit_box=None
                    chosen=best
                    reset_cand()
                    print(f"  f{fi:04d} EXIT RELINK ✓")

            # normal candidate accumulation
            if chosen is None:
