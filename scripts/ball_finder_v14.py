...
EXIT_RELINK_PX       = 200
EDGE_ARM_MARGIN      = 80
MIN_VEL_FOR_SIG      = 10.0   # rv2 < 此 → sig=0
MIN_AREA_RATIO       = 0.15   # box_area/ref_area < 此 → sig=0
MIN_SIG_THRESH       = 0.08   # A×B >= 此判定为 HIT
MERGE_WINDOW         = 2      # 前后N帧内合并为一组，取最大
TRAIL_LEN            = 60
...
def compute_sig(v1, v2, box, ref_area, A_max):
    """
    sig = A × B
    A = clip(real_acc / A_max, 0, 1)
    B = sin²(Δheading/2)，纯 t-2→t-1 vs t-1→t，无 smooth
    """
    area  = box_area(box)
...
while True:
    ...
    # S = A×B，thresh 线在 MIN_SIG_THRESH 处（已经是0-1量级）
