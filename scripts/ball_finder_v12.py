"""
ball_finder_v12.py
hit_signal = 加速度向量 L2 norm（方向+速度变化都计入）
"""
import cv2
import numpy as np
...
def hit_signal(v1, v2, fi=None):
    ...
    print(f"    SIG f{fi} |v1|={n1:.1f} |v2|={n2:.1f} acc={acc} |acc|={acc_norm:.1f}")
...
for fi,dets in enumerate(filtered):
    ...
    if trk_box is not None:
        sig_str=f" sig={sig:.1f}" if sig>0 else ""
...
while True:
    ...
    cv2.putText(frame,f"sig:{sv:.3f}",(x0,12),cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,200,255),1)
