...
OUTPUT_PATH = "/Users/harryzhang/git/tempopeak/outputs/ball_finder_v13.mp4"
...
MIN_ACC_FOR_HIT      = 30.0
MIN_SIG_THRESH       = 1000.0
COAST_DECAY          = 0.85
...
def hit_signal(v1, v2, fi=None):
    n1=np.linalg.norm(v1); n2=np.linalg.norm(v2)
    acc_vec=v2-v1; acc=float(np.linalg.norm(acc_vec))
    print(f"    SIG f{fi} |v1|={n1:.1f} |v2|={n2:.1f} |acc|={acc:.1f} sig={acc*n2:.1f}")
    if n1<MIN_VEL_FOR_HIT or n2<MIN_VEL_FOR_HIT:
        print(f"    → skip (|v1|<{MIN_VEL_FOR_HIT}:{n1<MIN_VEL_FOR_HIT}, |v2|<{MIN_VEL_FOR_HIT}:{n2<MIN_VEL_FOR_HIT})")
        return 0.0
    if acc<MIN_ACC_FOR_HIT:
        print(f"    → skip (|acc|={acc:.1f} < MIN_ACC={MIN_ACC_FOR_HIT})")
        return 0.0
    result=acc*n2
    print(f"    → HIT sig={result:.1f}")
    return result

def smooth_normalize(series, sigma=3):
...
def reset_track():
    global trk_box,trk_vel,trk_vel_prev,trk_coast,trk_vel_age,trk_coasting
...
for fi,dets in enumerate(filtered):
    ...
    if chosen is not None and not is_pred:
        ...
        if trk_vel_age>=3:
            sig=hit_signal(trk_vel_prev, new_vel, fi)
...
while True:
    ...
    if ex is not None:
        ...
        for d in low_unique[fi]:
            bc=center(d[:4])
            if float(np.linalg.norm(bc-np.array([ecx,ecy])))<=EXIT_SEARCH_R:
                x1,y1,x2,y2=map(int,d[:4])
                cv2.rectangle(frame,(x1,y1),(x2,y2),(255,80,0),1)
                cv2.putText(frame,f"{d[4]:.2f}",(x1,y1-4),cv2.FONT_HERSHEY_SIMPLEX,0.4,(255,80,0),1)
        for d in filtered[fi]:
            bc=center(d[:4])
            if float(np.linalg.norm(bc-np.array([ecx,ecy])))<=EXIT_SEARCH_R:
                x1,y1,x2,y2=map(int,d[:4])
                cv2.rectangle(frame,(x1,y1),(x2,y2),(0,255,255),1)
                cv2.putText(frame,f"{d[4]:.2f}",(x1,y1-4),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,255,255),1)
