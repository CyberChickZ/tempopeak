"""
ball_finder_v8.py
hit_signal = angle_change × new_velocity
抛物线：angle小 → signal小
击球：angle大 × vel大 → signal大
"""
import cv2
import numpy as np
...
OUTPUT_PATH = "/Users/harryzhang/git/tempopeak/outputs/ball_finder_v8.mp4"
...
def hit_signal(v1, v2):
    """angle_change(度) × new_speed(px/frame)"""
...
for fi,dets in enumerate(filtered):
    ...
    if trk_box is not None:
        angle_str=""
        if sig>0:
            n1=np.linalg.norm(trk_vel); n2=np.linalg.norm(
                center(chosen[:4])-center(trk_box) if chosen else trk_vel)
            angle_str=f" sig={sig:.0f}"
        print(f"  f{fi:04d} {'P' if is_pred else 'R'} "
              f"vel={np.linalg.norm(trk_vel):.1f} age={trk_vel_age}{angle_str}")
