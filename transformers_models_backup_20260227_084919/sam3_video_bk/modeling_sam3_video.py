...
class Sam3VideoInferenceSession:
    ...
    def __init__(
        ...
        self.output_buffer = []
        # Optional hook: external code can request forced memory update for (frame_idx,obj_id). No-op if unused.
        self.force_mask_memory_update = {}
        # Optional gate: external code can block memory updates for (frame_idx,obj_id). Default is allow.
        self.memory_update_allowed = {}
        # Pending memory candidates keyed by frame_idx, used only when delayed memory update is enabled.
    ...
    def set_force_memory_update(self, frame_idx: int, obj_id: int, mask: torch.Tensor):
        ...
        self.force_mask_memory_update[(frame_idx, obj_id)] = mask

    def pop_force_memory_update(self, frame_idx: int, obj_id: int):
        return self.force_mask_memory_update.pop((frame_idx, obj_id), None)

    def set_memory_update_allowed(self, frame_idx: int, obj_id: int, allowed: bool):
        self.memory_update_allowed[(frame_idx, obj_id)] = bool(allowed)

    def pop_memory_update_allowed(self, frame_idx: int, obj_id: int):
        return self.memory_update_allowed.pop((frame_idx, obj_id), None)
    ...
    def reset_tracking_data(self):
        ...
        self.force_mask_memory_update.clear()
        self.memory_update_allowed.clear()
        self.pending_memory_update.clear()
...
class Sam3VideoModel(Sam3VideoPreTrainedModel):
    def __init__(self, config: Sam3VideoConfig):
        ...
        self.delay_memory_update_until_verified = config.delay_memory_update_until_verified
        # hotstart parameters
    ...
    def _tracker_update_memories(
        ...
        pending_memory_update = self._build_pending_memory_update(
            inference_session=inference_session,
            frame_idx=frame_idx,
            low_res_masks=low_res_masks,
            reconditioned_masks=reconditioned_masks,
        )
        if pending_memory_update is None:
            return
        self._commit_pending_memory_update(
            inference_session=inference_session,
            frame_idx=frame_idx,
            pending_memory_update=pending_memory_update,
            apply_memory_gate=False,
        )

    def _build_pending_memory_update(
        self,
        inference_session: Sam3VideoInferenceSession,
        frame_idx: int,
        low_res_masks: Tensor,
        ...
        if len(inference_session.obj_ids) == 0 or low_res_masks.size(0) == 0:
            return None
        ...
        obj_ids = [int(obj_id) for obj_id in inference_session.obj_ids]
        prompt_ids = [inference_session.obj_id_to_prompt_id[obj_id] for obj_id in inference_session.obj_ids]
        high_res_masks_by_obj_id = {
            obj_id: high_res_masks[obj_idx : obj_idx + 1] for obj_idx, obj_id in enumerate(obj_ids)
        }
        return {
            "obj_ids": obj_ids,
            "prompt_ids": prompt_ids,
            "high_res_masks": high_res_masks_by_obj_id,
        }

    def _commit_pending_memory_update(
        self,
        inference_session: Sam3VideoInferenceSession,
        frame_idx: int,
        pending_memory_update: dict[str, Any],
        apply_memory_gate: bool = True,
    ):
        obj_ids = pending_memory_update.get("obj_ids", [])
        if len(obj_ids) == 0:
            return

        high_res_masks_by_obj_id = pending_memory_update.get("high_res_masks", {})
        prompt_ids = pending_memory_update.get("prompt_ids", [])

        candidate_masks = []
        candidate_obj_ids = []
        candidate_prompt_ids = []
        for obj_i, obj_id in enumerate(obj_ids):
            high_res_mask = high_res_masks_by_obj_id.get(obj_id)
            if high_res_mask is None:
                continue
            candidate_masks.append(high_res_mask)
            candidate_obj_ids.append(int(obj_id))
            if obj_i < len(prompt_ids):
                candidate_prompt_ids.append(prompt_ids[obj_i])
            else:
                candidate_prompt_ids.append(inference_session.obj_id_to_prompt_id.get(obj_id, -1))

        if len(candidate_masks) == 0:
            return

        high_res_masks = torch.cat(candidate_masks, dim=0)

        # Optional forced memory override (no-op unless populated).
        forced_flags = [False] * len(candidate_obj_ids)
        if inference_session.force_mask_memory_update:
            for obj_idx, obj_id in enumerate(candidate_obj_ids):
                forced = inference_session.pop_force_memory_update(frame_idx, int(obj_id))
                if forced is not None:
                    high_res_masks[obj_idx] = forced.to(
                        device=high_res_masks.device, dtype=high_res_masks.dtype, non_blocking=True
                    ).float()
                    forced_flags[obj_idx] = True

        # Apply non-overlapping constraints before memory encoding.
        # Constraints are enforced independently per prompt group.
        high_res_masks = self._suppress_object_pw_area_shrinkage(high_res_masks, candidate_prompt_ids)
        # Use mask areas as a proxy for object scores
        object_score_logits = torch.where((high_res_masks > 0).any(dim=(-1, -2)), 10.0, -10.0)

        if apply_memory_gate:
            obj_indices_to_encode = []
            for obj_idx, obj_id in enumerate(candidate_obj_ids):
                allowed = inference_session.pop_memory_update_allowed(frame_idx, int(obj_id))
                is_allowed = True if allowed is None else bool(allowed)
                if is_allowed or forced_flags[obj_idx]:
                    obj_indices_to_encode.append(obj_idx)
        else:
            obj_indices_to_encode = list(range(len(candidate_obj_ids)))

        if len(obj_indices_to_encode) == 0:
            return

        selected_high_res_masks = high_res_masks[obj_indices_to_encode]
        selected_object_score_logits = object_score_logits[obj_indices_to_encode].unsqueeze(-1)

        # Encode memories for selected objects in one batch call
        maskmem_features_batched, maskmem_pos_enc_batched = self.run_memory_encoder(
            inference_session,
            frame_idx,
            selected_high_res_masks,
            selected_object_score_logits,
        )

        for encoded_idx, global_obj_idx in enumerate(obj_indices_to_encode):
            obj_id = candidate_obj_ids[global_obj_idx]
            obj_idx = inference_session._obj_id_to_idx.get(obj_id, None)
            if obj_idx is None:
                continue
            output_dict = inference_session.output_dict_per_obj[obj_idx]
            maskmem_features = maskmem_features_batched[:, encoded_idx : encoded_idx + 1]
            maskmem_pos_enc = maskmem_pos_enc_batched[:, encoded_idx : encoded_idx + 1]
    ...
    def commit_frame_memory(
        self,
        inference_session: Sam3VideoInferenceSession,
        frame_idx: int,
        low_res_masks_for_all_objects: Tensor | None = None,
        reconditioned_masks: dict[int, Tensor] | None = None,
    ):
        if low_res_masks_for_all_objects is not None:
            inference_session.pending_memory_update.pop(frame_idx, None)
            pending_memory_update = self._build_pending_memory_update(
                inference_session=inference_session,
                frame_idx=frame_idx,
                low_res_masks=low_res_masks_for_all_objects,
                reconditioned_masks=reconditioned_masks,
            )
        else:
            pending_memory_update = inference_session.pending_memory_update.pop(frame_idx, None)
    ...
    def run_tracker_update_planning_phase(
        ...
        # Step 4: Prepare and optionally encode SAM2 memory on the current frame's prediction masks
        # (detection masks are used for reconditioned objects).
        batch_size = tracker_low_res_masks_global.size(0)
        if batch_size > 0:
            ...
            if self.delay_memory_update_until_verified:
                pending_memory_update = self._build_pending_memory_update(
                    inference_session=inference_session,
                    frame_idx=frame_idx,
                ...
                if pending_memory_update is not None:
                    inference_session.pending_memory_update[frame_idx] = pending_memory_update
            else:
    ...
    def _det_track_one_frame(
        ...
        # Memory encoding is either executed here (default) or deferred to explicit commit (optional).
